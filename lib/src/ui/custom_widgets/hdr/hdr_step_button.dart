...
class HdrStepButton extends StatelessWidget {
  final StepBracketValuesBloc stepBracketValuesBloc;

  const HdrStepButton({Key key, @required this.stepBracketValuesBloc})
      : super(key: key);
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<HdrStepBloc, bool>(
      builder: (context, state) {
        return TextButton(
            onPressed: () {
              context.read<HdrStepBloc>().add(true);
            },
            child: StreamBuilder(
              stream: stepBracketValuesBloc.currentStep,
              builder: (context, stateStep) {
                if (stateStep.hasError) {
                  return Text(
                    "${stateStep.error}",
                ...
                return Column(
                  children: [
                    Text(
                      stateStep.hasData
                          ? "${(stepBracketValuesBloc.flash ? '' : '±') + stateStep.data}"
                          : "",
